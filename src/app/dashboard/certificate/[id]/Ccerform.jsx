...
export default  function Formcer() {
  const [data, setdata] = useState([])
    const {id}=useParams()
useEffect(async()=>{
    const res=await axios.post(site_url+"getcertificate",{id}).then((res)=>{
     if(res.status==200){
...
{data ? (
   ...
   <div className="mailCr border text-center print:mt-20 print:border-none border-gray-800 px-4 py-6 mt-10 pb-10 mx-auto print:h-[500px]  w-[90%] print:w-[90%] md:w-[60%] rounded-md shadow-md">
       ...
       <div className='pt-1 pb-3'>
         <h1 className={` text-xl text-semibold text-emerald-500 uppercase`}>{data.candidateName}</h1>
       </div>
       ...
       <div className='py-1'>
         <h1 className='text-md capitalize'>{data.candidateTitle}</h1>
       </div>
       <div className='pt-4'>
